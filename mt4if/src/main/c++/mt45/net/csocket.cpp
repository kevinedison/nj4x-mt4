...
string CSocket::getLocalAddress() throw(SocketException)
{
	...
	if (getsockname(sockDesc, reinterpret_cast<sockaddr *>(&addr), reinterpret_cast<socklen_t *>(&addr_len)) < 0)
	{
...
throw(SocketException) : CSocket(type, protocol)
{
}
...
throw(SocketException)
{
	...
	if (getpeername(sockDesc, reinterpret_cast<sockaddr *>(&addr), reinterpret_cast<socklen_t *>(&addr_len)) < 0)
	{
...
throw(SocketException) : CSocket(SOCK_STREAM, IPPROTO_TCP)
{
	...
	CommunicatingSocket(SOCK_DGRAM, IPPROTO_UDP)
{
	...
	//setsockopt(sockDesc, SOL_SOCKET, SO_RCVTIMEO, (raw_type *) &timeoutMillis, sizeof(timeoutMillis));
	//setsockopt(sockDesc, SOL_SOCKET, SO_SNDTIMEO, (raw_type *) &timeoutMillis, sizeof(timeoutMillis));
}
...
void UDPSocket::setBroadcast()
{
	...
	setsockopt(sockDesc, SOL_SOCKET, SO_BROADCAST,
	           reinterpret_cast<raw_type *>(&broadcastPermission), sizeof(broadcastPermission));
}
...
throw(SocketException)
{
	...
	if (sendto(sockDesc, (raw_type *)buffer, bufferLen, 0,
	           reinterpret_cast<sockaddr *>(&destAddr), sizeof(destAddr)) != bufferLen)
	{
                        ...
                        unsigned short& sourcePort) throw(SocketException)
{
	...
	int rtn;
	if ((rtn = recvfrom(sockDesc, static_cast<raw_type *>(buffer), bufferLen, 0,
	                    reinterpret_cast<sockaddr *>(&clntAddr), static_cast<socklen_t *>(&addrLen))) < 0)
	{
...
void UDPSocket::setMulticastTTL(unsigned char multicastTTL) throw(SocketException)
{
	if (setsockopt(sockDesc, IPPROTO_IP, IP_MULTICAST_TTL,
	               reinterpret_cast<raw_type *>(&multicastTTL), sizeof(multicastTTL)) < 0)
	{
...
void UDPSocket::joinGroup(const string& multicastGroup) throw(SocketException)
{
	...
	if (setsockopt(sockDesc, IPPROTO_IP, IP_ADD_MEMBERSHIP,
	               reinterpret_cast<raw_type *>(&multicastRequest),
